...
class DataManager:
    def __init__(self, configs, logger):
        ...
        self.embedding_dim = configs.embedding_dim            # 200

        self.vocabs_dir = configs.vocabs_dir                  # 词典和标签
        self.token2id_file = self.vocabs_dir + "/token2id"
        self.label2id_file = self.vocabs_dir + "/label2id"

        self.token2id, self.id2token, self.label2id, self.id2label = self.loadVocab()  # 格式为字典

        self.max_token_number = len(self.token2id)
        self.max_label_number = len(self.label2id)

        jieba.load_userdict(self.token2id.keys())                # 给jieba传词典
    ...
    def loadVocab(self):
        ...
        token2id = {}
        id2token = {}
        with open(self.token2id_file, 'r', encoding='utf-8') as infile:        # 读token
            for row in infile:
                row = row.rstrip()
                token = row.split('\t')[0]
    ...
    def buildVocab(self, train_path):
        # train_path = 'D:\\pycharm_project\\sequence-labeling-BiLSTM-CRF-master\\data\\example_datasets3\\train.csv'
        # df_train = read_csv_(train_path, names=["token", "label"], delimiter='d')
        ...
        labels = list(set(df_train["label"][df_train["label"].notnull()]))                  # 排序
        token2id = dict(zip(tokens, range(1, len(tokens) + 1)))                             # 编号
    ...
    # def saveVocab(id2token, id2label):
    #     with open("D:\\pycharm_project\\sequence-labeling-BiLSTM-CRF-master\\data\\example_datasets3\\vocabs\\token2id", "w", encoding='utf-8') as outfile:
    #         for idx in id2token:
    #             outfile.write(id2token[idx] + "\t" + str(idx) + "\n")
    #
    #     with open("D:\\pycharm_project\\sequence-labeling-BiLSTM-CRF-master\\data\\example_datasets3\\vocabs\\label2id", "w", encoding='utf-8') as outfile:
    #         for idx in id2label:
    ...
    def getEmbedding(self, embed_file):
        ...
        emb_matrix[self.token2id[self.PADDING], :] = np.zeros(shape=(self.embedding_dim))                       # 全为零，（总词数，200）
        ...
        with open(embed_file, "r", encoding="utf-8") as infile:
            for row in infile:
                ...
                assert self.embedding_dim == len(
                    items[1:]), "embedding dim must be consistent with the one in `token_emb_dir'."             # 向量维度统一
                emb_vec = np.array([float(val) for val in items[1:]])                                           # 取词向量
                if token in self.token2id.keys():
                    emb_matrix[self.token2id[token], :] = emb_vec                                               # 嵌入词向量，一个编号对应一个向量

        return emb_matrix

    def nextBatch(self, X, y, start_index):
        last_index = start_index + self.batch_size              # 批次终点
        X_batch = list(X[start_index:min(last_index, len(X))])  # 截取数据
        y_batch = list(y[start_index:min(last_index, len(X))])
        if last_index > len(X):
            left_size = last_index - (len(X))
            for i in range(left_size):                 # 不满32句话 补满32句话
    ...
    def prepare(self, tokens, labels, is_padding=True, return_psyduo_label=False):
        ...
        for record in zip(tokens, labels):                     # 字和标签一一对应
            c = record[0]
            l = record[1]
            if c == -1:  # empty line   一句话结束，句号后是空格
                if len(tmp_x) <= self.max_sequence_length:     # 300  长度小于300则作为一个列表添加
                    X.append(tmp_x)
                    y.append(tmp_y)
        ...
        if is_padding:
            X = np.array(self.padding(X))                                            # 补零至300维
        else:
            X = np.array(X)
    ...
    def getTrainingSet(self, train_val_ratio=0.9):
        ...
        df_train["token_id"] = df_train.token.map(lambda x: -1 if str(x) == str(np.nan) else self.token2id[x])   # 读取文本序列对应的编号
        df_train["label_id"] = df_train.label.map(lambda x: -1 if str(x) == str(np.nan) else self.label2id[x])   # 读取标签序列对应的编号

        # convert the data in maxtrix  以句为分隔的矩阵  序号代表文本和标签
        X, y = self.prepare(df_train["token_id"], df_train["label_id"])                            # X和y为二维列表，一句话为一行

        # shuffle the samples   句子顺序打乱
        num_samples = len(X)
        ...
        X_train = X[:int(num_samples * train_val_ratio)]             # 长度为句子数量
    ...
    def getTestingSet(self):
        ...
        df_test["token_id"] = df_test.token.map(lambda x: self.mapFunc(x, self.token2id))  #获取词典 序号
